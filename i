...
TabLayout.Padding=UDim.new(0,6)
    ...
    local resizeMinWidth=320
    local resizeMinHeight=220
    ...
    UIS.InputChanged:Connect(function(i)
        ...
        local uiScale=math.max(Scale.Scale,0.01)
        local d=(i.Position-resizeStart)/uiScale
        local w=resizeSize.X
        local h=resizeSize.Y
        local px=Frame.Position.X.Offset
        local py=Frame.Position.Y.Offset

        if resizeCorner=="ResizeBottomRight" then
            w=math.max(resizeMinWidth,resizeSize.X+d.X)
            h=math.max(resizeMinHeight,resizeSize.Y+d.Y)
        elseif resizeCorner=="ResizeBottomLeft" then
            w=math.max(resizeMinWidth,resizeSize.X-d.X)
            h=math.max(resizeMinHeight,resizeSize.Y+d.Y)
            if w>resizeMinWidth then
                px=px+d.X
            else
                px=px+(resizeSize.X-resizeMinWidth)
            end
        elseif resizeCorner=="ResizeTopRight" then
            w=math.max(resizeMinWidth,resizeSize.X+d.X)
            h=math.max(resizeMinHeight,resizeSize.Y-d.Y)
            if h>resizeMinHeight then
                py=py+d.Y
            else
                py=py+(resizeSize.Y-resizeMinHeight)
            end
...
local function UpdateWindowLayout()
    local frameWidth=math.max(resizeMinWidth or 320,Frame.Size.X.Offset)
    local frameHeight=math.max(resizeMinHeight or 220,Frame.Size.Y.Offset)
