...
TabLayout.Padding=UDim.new(0,6)
    ...
    Credits.Position=UDim2.new(.91,0,.05,0)
    ...
    local function MakeResizeCorner(name,x,y,ax,ay)
        local Handle=Instance.new("TextButton")
        Handle.Name=name
        Handle.Parent=Frame
        Handle.BackgroundTransparency=1
        Handle.BorderSizePixel=0
        Handle.Position=UDim2.new(x,0,y,0)
        Handle.Size=UDim2.new(0,16,0,16)
        Handle.AnchorPoint=Vector2.new(ax,ay)
        Handle.Text=""
    ...
    MakeResizeCorner("ResizeTopLeft",0,0,0,0)
    MakeResizeCorner("ResizeTopRight",1,0,1,0)
    MakeResizeCorner("ResizeBottomLeft",0,1,0,1)
    MakeResizeCorner("ResizeBottomRight",1,1,1,1)

    UIS.InputChanged:Connect(function(i)
        if not resizing then return end
        if i~=resizeInput and i.UserInputType~=Enum.UserInputType.MouseMovement and i.UserInputType~=Enum.UserInputType.Touch then return end
        local d=i.Position-resizeStart
        local w,h=resizeSize.X,resizeSize.Y
        local px,py=Frame.Position.X.Offset,Frame.Position.Y.Offset
        if resizeCorner=="ResizeBottomRight" then
            w=math.max(320,resizeSize.X+d.X); h=math.max(220,resizeSize.Y+d.Y)
        elseif resizeCorner=="ResizeBottomLeft" then
            w=math.max(320,resizeSize.X-d.X); h=math.max(220,resizeSize.Y+d.Y); px=px+d.X
        elseif resizeCorner=="ResizeTopRight" then
            w=math.max(320,resizeSize.X+d.X); h=math.max(220,resizeSize.Y-d.Y); py=py+d.Y
        elseif resizeCorner=="ResizeTopLeft" then
            w=math.max(320,resizeSize.X-d.X); h=math.max(220,resizeSize.Y-d.Y); px=px+d.X; py=py+d.Y
        end
        Frame.Size=UDim2.new(0,w,0,h)
        Frame.Position=UDim2.new(Frame.Position.X.Scale,px,Frame.Position.Y.Scale,py)
        UpdateWindowLayout()
    end)
...
local function UpdateWindowLayout()
    ...
    local frameWidth=math.max(320,Frame.AbsoluteSize.X)
    local frameHeight=math.max(220,Frame.AbsoluteSize.Y)
    local contentHeight=math.max(158,frameHeight-62)
    local dashWidth=math.clamp(math.floor(frameWidth*.255),100,180)
    local pagesWidth=math.max(170,frameWidth-dashWidth-21)

    Dash.Size=UDim2.new(0,dashWidth,0,contentHeight)
    Tabs.Size=UDim2.new(0,math.max(92,dashWidth-7),0,contentHeight-9)
    Pages.Position=UDim2.new(0,dashWidth+11,0,.168)
    Pages.Size=UDim2.new(0,pagesWidth,0,contentHeight)
    ...
    local function RegisterConfig(name,typ,getter,setter,resetter)
        local entry={
            Name=tostring(name or ""),
            Type=typ,
            Get=getter,
...
do
    ...
    StatsPanel.Position=UDim2.new(.5,0,.2,0)
    ...
    StatsButton.BackgroundColor3=Color3.fromRGB(30,30,30)
    StatsButton.BorderSizePixel=0
    StatsButton.Position=UDim2.new(.85,0,.05,0)
    StatsButton.Size=UDim2.new(0,22,0,22)
    StatsButton.Font=Enum.Font.GothamBold
    StatsButton.Text="..."
    StatsButton.TextColor3=Color3.new(1,1,1)
    StatsButton.TextSize=11
    StatsButton.ZIndex=50
    StatsButtonCorner.CornerRadius=UDim.new(0,5)
