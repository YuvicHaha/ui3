...
do
    ...
    Palette.Parent=Gui
    Palette.BackgroundColor3=Color3.fromRGB(17,17,17)
    Palette.BorderSizePixel=0
    Palette.AnchorPoint=Vector2.new(.5,.5)
    Palette.Position=UDim2.new(.5,0,.5,0)
    Palette.Size=UDim2.new(0,370,0,310)
    ...
    PS.Color=Color3.fromRGB(45,45,45)
    PS.Transparency=.05
    PS.Parent=Palette

    PTitle.Parent=Palette
    PTitle.BackgroundTransparency=1
    PTitle.Position=UDim2.new(0,14,0,10)
    PTitle.Size=UDim2.new(1,-28,0,22)
    PTitle.Font=Enum.Font.GothamSemibold
    PTitle.Text="Command Palette"
    PTitle.TextColor3=Color3.new(1,1,1)
    PTitle.TextSize=13
    PTitle.TextXAlignment=Enum.TextXAlignment.Left
    PTitle.ZIndex=501

    PHint.Parent=Palette
    PHint.BackgroundTransparency=1
    PHint.Position=UDim2.new(0,14,0,31)
    PHint.Size=UDim2.new(1,-28,0,16)
    PHint.Font=Enum.Font.Gotham
    PHint.Text="Search any feature by name"
    PHint.TextColor3=Color3.fromRGB(120,120,120)
    PHint.TextSize=9
    PHint.TextXAlignment=Enum.TextXAlignment.Left
    PHint.ZIndex=501

    PSearch.Parent=Palette
    PSearch.BackgroundColor3=Color3.fromRGB(10,10,10)
    PSearch.BorderSizePixel=0
    PSearch.Position=UDim2.new(0,12,0,55)
    PSearch.Size=UDim2.new(1,-24,0,30)
    PSearch.Font=Enum.Font.GothamSemibold
    PSearch.PlaceholderText="Type a feature..."
    PSearch.PlaceholderColor3=Color3.fromRGB(90,90,90)
    PSearch.Text=""
    PSearch.TextColor3=Color3.new(1,1,1)
    PSearch.TextSize=10
    ...
    PP.PaddingLeft=UDim.new(0,9)
    PP.PaddingRight=UDim.new(0,9)
    PP.Parent=PSearch

    PList.Parent=Palette
    PList.BackgroundTransparency=1
    PList.BorderSizePixel=0
    PList.Position=UDim2.new(0,12,0,94)
    PList.Size=UDim2.new(1,-24,1,-106)
    PList.CanvasSize=UDim2.new(0,0,0,0)
    PList.ScrollBarThickness=3
    PList.ScrollBarImageColor3=Color3.fromRGB(70,70,70)
    PList.ZIndex=501

    PPadding.Parent=PList
    ...
    local function ClearPaletteResults()
    ...
    local function AddPaletteResult(data,index)
        local B=Instance.new("TextButton")
        local BC=Instance.new("UICorner")
        ...
        B.AutoButtonColor=false
        B.Font=Enum.Font.GothamSemibold
        B.Text=tostring(data.Name or "Feature")
        B.TextColor3=Color3.new(1,1,1)
        B.TextSize=10
        ...
        sub.Text=tostring(data.Type or "Feature")
        ...
        B.MouseEnter:Connect(function()
            B.BackgroundColor3=Color3.fromRGB(30,60,64)
        end)
        ...
        B.MouseButton1Click:Connect(function()
            ...
            if page and page.Parent then
                ...
                for _,tab in pairs(PageTabs) do
                    if tab:IsA("GuiButton") then
                        tab.TextTransparency=tab.Name=="Tab" and (tab.Text==page.Name and 0 or .3) or tab.TextTransparency
                        if tab.Text==page.Name then
                            tab.BackgroundColor3=Color3.fromRGB(25,25,25)
                            tab.TextTransparency=0
                        end
                    end
                ...
                task.defer(function()
                    if data.Object and data.Object.Parent then
                        local y=data.Object.AbsolutePosition.Y-page.AbsolutePosition.Y+page.CanvasPosition.Y-12
                        ...
                        task.delay(.55,function()
                            ...
                            and HighlightedObject==data.Object
                            and data.Object.Parent
                            and HighlightedColor then
                                data.Object.BackgroundColor3=HighlightedColor
                                HighlightedObject=nil
                                HighlightedColor=nil
                            end
                        end)
    ...
    local function RefreshPalette()
        ...
        local query=tostring(PSearch.Text or ""):lower()
        ...
        for i,data in ipairs(matches) do
            ...
            AddPaletteResult(data,i)
        end

        PList.CanvasSize=UDim2.new(0,0,0,math.max(PLayout.AbsoluteContentSize.Y+6,PList.AbsoluteSize.Y))
    end
    ...
    OpenCommandPalette=function()
        ...
        if PaletteOpen then
            ...
            PSearch:ReleaseFocus()
        end
    end

    PSearch:GetPropertyChangedSignal("Text"):Connect(RefreshPalette)

    PSearch.FocusLost:Connect(function(_,input)
    ...
    UIS.InputBegan:Connect(function(i,gp)
        ...
        if i.KeyCode==Enum.KeyCode.Escape and PaletteOpen then
            PaletteOpen=false
            Palette.Visible=false
            PSearch:ReleaseFocus()
        end
    ...
    PTitle.InputChanged:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseMovement
        or i.UserInputType==Enum.UserInputType.Touch then
            pi=i
        end
...
do
    ...
    StatsPanel.Position=UDim2.new(1,-190,0,12)
    StatsPanel.Size=UDim2.new(0,178,0,145)
    StatsPanel.Active=true
    StatsPanel.ZIndex=220

    SC.CornerRadius=UDim.new(0,7)
    SC.Parent=StatsPanel

    SS.Color=Color3.fromRGB(35,35,35)
    SS.Transparency=.1
    SS.Parent=StatsPanel

    ST.Parent=StatsPanel
    ST.BackgroundTransparency=1
    ST.Position=UDim2.new(0,10,0,8)
    ST.Size=UDim2.new(1,-20,1,-16)
    ST.Font=Enum.Font.Code
    ST.TextColor3=Color3.fromRGB(225,225,225)
    ST.TextSize=10
    ST.TextXAlignment=Enum.TextXAlignment.Left
    ST.TextYAlignment=Enum.TextYAlignment.Top
    ST.TextWrapped=false
    ST.ZIndex=221
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if now-lastFpsTime>=.5 then
            fps=math.floor(frameCount/(now-lastFpsTime)+.5)
            frameCount=0
            lastFpsTime=now
        end
        ...
        local server=job~="" and job:sub(1,6) or "local"

        ST.Text=
            "FPS: "..tostring(fps).."\n"
            .."Ping: "..tostring(GetPing()).."ms\n"
            .."Players: "..tostring(count).."/"..tostring(maxPlayers).."\n"
            .."Server: "..server.."\n"
            .."Session Time: "..FormatTime(now-startTime).."\n"
            .."Players Joined: "..tostring(joined).."\n"
            .."Players Left: "..tostring(left)
    end)
