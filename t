...
local function UpdateWindowLayout()
    ...
    function PageYep:addPage(
        ...
        --// DRAG-OUT SHORTCUTS
        local function AttachShortcutDrag(source,name,activate,getState)
            ...
            local holdToken=0
            local shortcut=nil
            local shortcutLabel=nil
            local shortcutStroke=nil
            local sourceDefault=source.BackgroundColor3
            local shortcutMoveConn=nil
            local shortcutEndConn=nil
            local shortcutClickConn=nil

            local function setReady(on)
                if not source or not source.Parent then return end
                source.BackgroundColor3=on and Color3.fromRGB(28,52,55) or sourceDefault
                local stroke=source:FindFirstChildOfClass("UIStroke")
                if stroke then
                    stroke.Color=on and Color3.fromRGB(90,190,195) or Color3.fromRGB(45,45,45)
                    stroke.Transparency=on and .05 or .15
                end
            end

            local function updateShortcut()
                if not shortcut or not shortcut.Parent then return end
                local state=getState and getState()
                if state~=nil then
                    shortcutLabel.Text=(name or "Shortcut")..(state and "  • ON" or "  • OFF")
                    shortcut.BackgroundColor3=state and Color3.fromRGB(35,90,70) or Color3.fromRGB(28,28,28)
                    shortcutLabel.TextColor3=state and Color3.fromRGB(225,255,235) or Color3.fromRGB(220,220,220)
                end
            end

            local function disconnectShortcut()
                if shortcutMoveConn then shortcutMoveConn:Disconnect();shortcutMoveConn=nil end
                if shortcutEndConn then shortcutEndConn:Disconnect();shortcutEndConn=nil end
                if shortcutClickConn then shortcutClickConn:Disconnect();shortcutClickConn=nil end
            end
            ...
            local function removeShortcut()
                ...
                source:SetAttribute("ShortcutHolding",false)
                setReady(false)
                disconnectShortcut()
                if shortcut and shortcut.Parent then shortcut:Destroy() end
                shortcut=nil
                shortcutLabel=nil
                shortcutStroke=nil
            end
            ...
            local function beginShortcutDrag(input)
                ...
                local moved=false
                local offset=Vector2.new(shortcut.AbsoluteSize.X/2,shortcut.AbsoluteSize.Y/2)
                local dragActive=true

                disconnectShortcut()
                shortcutMoveConn=UIS.InputChanged:Connect(function(changed)
                    if not dragActive or not shortcut or not shortcut.Parent then return end
                    ...
                    local pos=changed.Position
                    if (pos-start).Magnitude>=3 then moved=true end
                    if not moved then return end
                    shortcut.Position=UDim2.fromOffset(
                        math.clamp(pos.X-offset.X,4,math.max(4,Gui.AbsoluteSize.X-shortcut.AbsoluteSize.X-4)),
                        math.clamp(pos.Y-offset.Y,4,math.max(4,Gui.AbsoluteSize.Y-shortcut.AbsoluteSize.Y-4))
                    )
                end)
                shortcutEndConn=UIS.InputEnded:Connect(function(ended)
                    if ended~=input then return end
                    dragActive=false
                    if shortcutMoveConn then shortcutMoveConn:Disconnect();shortcutMoveConn=nil end
                    if shortcutEndConn then shortcutEndConn:Disconnect();shortcutEndConn=nil end
                    if not moved and shortcut and shortcut.Parent then
                        pcall(activate)
                        updateShortcut()
                    end
                end)
            ...
            local function createShortcut(input)
                if shortcut and shortcut.Parent then
                    shortcut.Position=UDim2.fromOffset(
                        math.clamp(input.Position.X-66,4,math.max(4,Gui.AbsoluteSize.X-136)),
                        math.clamp(input.Position.Y-15,4,math.max(4,Gui.AbsoluteSize.Y-34))
                    )
                    return
                end
                ...
                shortcut=Instance.new("TextButton")
                shortcut.Name=tostring(name or "Shortcut").."_Shortcut"
                shortcut.Parent=Gui
                shortcut.Size=UDim2.new(0,132,0,30)
                shortcut.Position=UDim2.fromOffset(
                    math.clamp(input.Position.X-66,4,math.max(4,Gui.AbsoluteSize.X-136)),
                    math.clamp(input.Position.Y-15,4,math.max(4,Gui.AbsoluteSize.Y-34))
                )
                shortcut.BackgroundColor3=Color3.fromRGB(28,28,28)
                ...
                corner.Parent=shortcut

                shortcutStroke=Instance.new("UIStroke")
                shortcutStroke.Color=Color3.fromRGB(75,75,75)
                shortcutStroke.Transparency=.1
                shortcutStroke.Parent=shortcut

                shortcutLabel=Instance.new("TextLabel")
                shortcutLabel.Parent=shortcut
                shortcutLabel.BackgroundTransparency=1
                shortcutLabel.Size=UDim2.new(1,-10,1,0)
                shortcutLabel.Position=UDim2.new(0,5,0,0)
                shortcutLabel.Font=Enum.Font.GothamSemibold
                shortcutLabel.TextSize=10
                shortcutLabel.TextColor3=Color3.fromRGB(220,220,220)
                shortcutLabel.Text=name or "Shortcut"
                shortcutLabel.TextXAlignment=Enum.TextXAlignment.Center
                shortcutLabel.ZIndex=501
                shortcutLabel.Active=false

                table.insert(ShortcutEntries,{Object=shortcut,Source=source})
                updateShortcut()

                shortcut.InputBegan:Connect(function(i)
            ...
            source.InputBegan:Connect(function(input)
                ...
                if shortcut and shortcut.Parent then return end
                ShortcutDragToken+=1
                local token=ShortcutDragToken
                holding=true
                activeInput=input
                source:SetAttribute("ShortcutHolding",true)
                setReady(true)

                task.delay(.38,function()
                    if not holding or activeInput~=input or token~=ShortcutDragToken then return end
                    holding=false
                    activeInput=nil
                    source:SetAttribute("ShortcutHolding",false)
                    setReady(false)
            ...
            return {Update=updateShortcut,Remove=removeShortcut}
        end
        ...
        function Elements:addButton(name,callback)
            ...
            B.MouseButton1Click:Connect(function()
                if H:GetAttribute("ShortcutDragging") or H:GetAttribute("ShortcutHolding") then
                    return
                end
        ...
        function Elements:addToggle(name,a,b,c,d,e,f)
            ...
            local function UpdateSwitchVisual()
                local on=keybindMode and active or active
                TF.BackgroundColor3=
                    on
            ...
            local Shortcut=AttachShortcutDrag(
                H,
                name,
                function()
                    if keybindMode then
    ...
    local function ResetAll()
        ...
        ShortcutDragToken+=1
        for i=#ShortcutEntries,1,-1 do
            local item=ShortcutEntries[i]
            if item.Object and item.Object.Parent then
                item.Object:Destroy()
            end
            table.remove(ShortcutEntries,i)
        end
        for _,element in ipairs(AllElements) do
            if element.Object and element.Object.Parent then
                element.Object:SetAttribute("ShortcutDragging",false)
            end
        end
